...
queue queue_new() {
    ...
    q->length = 0;
    q->tail = q->head = NULL;
    // We add a separator to distinguish undo and redo moves
    // Undo moves will be poped from the top
    // Redo moves will be poped from the tail
    queue_push_head(q, create_move(-1, -1, 0));
    return q;
}
...
move_m create_move(int i, int j, square s) {
    move_m move = malloc(sizeof(struct move_m));
    move->i = i;
    move->j = j;
    move->s = s;
    return move;
...
{
  assert(q);
  return (queue_peek_tail(q)->i== queue_peek_head(q)->i==-1);
}*/
