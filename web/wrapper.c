...
game new_empty( uint i, uint j, bool wrapping, bool unique) {
    return game_new_empty_ext( i, j, wrapping, unique);
}
...
EMSCRIPTEN_KEEPALIVE
bool check_move(cgame g, uint i, uint j, square s) {
    return game_check_move(g, i, j, s);
}
...
EMSCRIPTEN_KEEPALIVE
bool is_immutable(cgame g, uint i, uint j) {
    return game_is_immutable(g, i, j);
}

EMSCRIPTEN_KEEPALIVE
bool is_equal(cgame g1, cgame g2) {
...
EMSCRIPTEN_KEEPALIVE
game default_solution(void) { return game_default_solution(); }
