...
Env* init(SDL_Window* win, SDL_Renderer* ren, int argc, char* argv[]) {
  ...
  SDL_Color color = {255, 0, 0}; /* blue color in RGBA */
  TTF_Font* font = TTF_OpenFont(FONT, FONTSIZE);
  if (!font) ERROR("TTF_OpenFont: %s\n", FONT);
  TTF_SetFontStyle(font, TTF_STYLE_BOLD);                                     // TTF_STYLE_ITALIC | TTF_STYLE_NORMAL
  SDL_Surface* surf = TTF_RenderText_Blended(font, "Projet MBacle!", color);  // blended rendering for ultra nice text
...
void render(SDL_Window* win, SDL_Renderer* ren, Env* env) {
  ...
  SDL_SetRenderDrawColor(ren, 0, 0, 255, SDL_ALPHA_OPAQUE); /* red */
  SDL_RenderDrawLine(ren, 120, 200, 100, 200);
  SDL_RenderDrawLine(ren, 100, 200, 100, 230);
  SDL_RenderDrawLine(ren, 100, 230, 120, 200);

  /* render a circle with points */
  const float pi = 3.14159265358979323846264338327950288419716939937510;
  float step = 2 * pi / 300; /* smoothing */
  for (float theta = 0.0; theta <= 2 * pi; theta += step) {
    int x1 = 100 + 50 * cosf(theta) + 0.5;
    int y1 = 200 + 50 * sinf(theta) + 0.5;
